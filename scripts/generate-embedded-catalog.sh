...
if [[ -n "$GENERATION_STORE_PATH" && "$GENERATION_STORE_PATH" != /* ]]; then
  GENERATION_STORE_PATH="${PWD}/${GENERATION_STORE_PATH}"
fi
...
if [[ -n "$GENERATION_STORE_PATH" ]]; then
  UPDATE_ARGS+=(--catalog-store-path "$GENERATION_STORE_PATH")
fi
