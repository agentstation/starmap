...
func TestProviderArchitectureRecoveryPreservesPresence(t *testing.T) {
	for _, scenario := range []struct {
		name, prior, current, field string
		path                        []string
	}{
		{name: "quantized/unknown", prior: `{"metadata":{"architecture":{"quantized":null}}}`, current: `{}`, field: "metadata.architecture.quantized", path: []string{"metadata", "architecture", "quantized"}},
		{name: "quantized/false", prior: `{"metadata":{"architecture":{"quantized":false}}}`, current: `{}`, field: "metadata.architecture.quantized", path: []string{"metadata", "architecture", "quantized"}},
		{name: "quantized/true", prior: `{"metadata":{"architecture":{"quantized":true}}}`, current: `{}`, field: "metadata.architecture.quantized", path: []string{"metadata", "architecture", "quantized"}},
		{name: "fine_tuned/unknown", prior: `{"metadata":{"architecture":{"fine_tuned":null}}}`, current: `{}`, field: "metadata.architecture.fine_tuned", path: []string{"metadata", "architecture", "fine_tuned"}},
		{name: "fine_tuned/false", prior: `{"metadata":{"architecture":{"fine_tuned":false}}}`, current: `{}`, field: "metadata.architecture.fine_tuned", path: []string{"metadata", "architecture", "fine_tuned"}},
		{name: "fine_tuned/true", prior: `{"metadata":{"architecture":{"fine_tuned":true}}}`, current: `{}`, field: "metadata.architecture.fine_tuned", path: []string{"metadata", "architecture", "fine_tuned"}},
	} {
		for _, replace := range []bool{false, true} {
			...
			t.Run(scenario.name+"/"+mode, func(t *testing.T) {
				...
				observe := func(partial bool) sources.Observation {
					...
					model.Metadata.Architecture = nil
					if fields.Metadata != nil {
						model.Metadata.Architecture = fields.Metadata.Architecture
					}
