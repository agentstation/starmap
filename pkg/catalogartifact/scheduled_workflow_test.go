...
func TestScheduledGenerationWorkflowPublishesOnlyValidatedChangedPayload(t *testing.T) {
	...
	for _, required := range []string{
		...
		`gh release create "${{ steps.change.outputs.tag }}"`,
	} {
	...
	if refresh < 0 || !(refresh < classify && classify < validate && validate < stage && stage < publish) {
		t.Fatalf("workflow order refresh/classify/validate/stage/publish = %d/%d/%d/%d/%d", refresh, classify, validate, stage, publish)
	}
