...
func TestEnhancerPipeline(t *testing.T) {
	...
	pipeline := reconcile.NewEnhancerPipeline(
...
func TestEnhancerPipelineWithErrors(t *testing.T) {
	...
	pipeline := reconcile.NewEnhancerPipeline(failingEnhancer, successEnhancer)
...
func TestEnhancerBatch(t *testing.T) {
	...
	pipeline := reconcile.NewEnhancerPipeline(enhancer)
	...
	enhanced, err := pipeline.EnhanceBatch(ctx, models)
	if err != nil {
...
func TestEnhancerCanEnhance(t *testing.T) {
	...
	pipeline := reconcile.NewEnhancerPipeline(selectiveEnhancer)
...
func TestReconcilerWithEnhancers(t *testing.T) {
	...
	// Create reconciler with enhancer
	r, err := reconcile.New(reconcile.WithEnhancers(enhancer))
	if err != nil {
	...
	// Reconcile models
	models, _, err := r.ReconcileModels(sources)
	if err != nil {
		t.Fatalf("ReconcileModels failed: %v", err)
	}
	...
	if models[0].Pricing == nil || models[0].Pricing.Tokens == nil ||
		models[0].Pricing.Tokens.Input == nil || models[0].Pricing.Tokens.Input.Per1M != 5.0 {
		t.Error("Expected model to be enhanced with pricing")
	}
}
...
func TestChainEnhancer(t *testing.T) {
	...
	// Create chain
	chain := reconcile.NewChainEnhancer(100, enhancer1, enhancer2)
...
func BenchmarkEnhancerPipeline(b *testing.B) {
	...
	pipeline := reconcile.NewEnhancerPipeline(enhancers...)
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, err := pipeline.EnhanceBatch(ctx, models)
		if err != nil {
