...
func (p *EnhancerPipeline) Enhance(ctx context.Context, model catalogs.Model) (catalogs.Model, error) {
	...
	for _, enhancer := range p.enhancers {
		...
		if p.tracker != nil {
			p.trackEnhancement(enhanced, result, enhancer)
		}
...
func (p *EnhancerPipeline) EnhanceBatch(ctx context.Context, models []catalogs.Model) ([]catalogs.Model, error) {
	...
	for _, enhancer := range p.enhancers {
		...
		for i, idx := range indices {
			if p.tracker != nil {
				p.trackEnhancement(models[idx], results[i], enhancer)
			}
...
// trackEnhancement tracks provenance for model enhancements
func (p *EnhancerPipeline) trackEnhancement(original, enhanced catalogs.Model, enhancer Enhancer) {
	...
	if original.Pricing == nil && enhanced.Pricing != nil {
		p.tracker.Track(
			...
			"pricing",
			ProvenanceInfo{
				Source:    SourceName(enhancer.Name()),
				Field:     "pricing",
...
type ModelsDevEnhancer struct {
	priority int
	data     map[string]interface{} // Simplified for example
}
...
func (e *ModelsDevEnhancer) Enhance(ctx context.Context, model catalogs.Model) (catalogs.Model, error) {
	...
	if enhanced.Pricing == nil {
		if data, exists := e.data[model.ID]; exists {
			...
			if pricing, ok := data.(map[string]interface{})["pricing"]; ok {
