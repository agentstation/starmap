...
type Conflict struct {
	...
	Ours      any    // Our change
	Theirs    any    // Their change
	Type      ConflictType
	CanMerge  bool // Whether automatic merge is possible
	Suggested any  // Suggested resolution
}
...
func NewThreeWayMerger(authorities authority.Authority, strategy Strategy) ThreeWayMerger {
...
func (m *threeWayMerger) WithProvenance(tracker provenance.Tracker) *threeWayMerger {
	m.tracker = tracker
	return m
}
...
func (m *threeWayMerger) MergeModels(base, ours, theirs catalogs.Model) (catalogs.Model, []Conflict, error) {
	// Create a proper deep copy of ours
	merged := catalogs.Model{
		ID:          ours.ID,
		Name:        ours.Name,
		Description: ours.Description,
		Authors:     ours.Authors,
		Metadata:    ours.Metadata,
		Features:    ours.Features,
		Limits:      ours.Limits,
		Pricing:     ours.Pricing,
		Generation:  ours.Generation,
	...
	if nameConflict := m.detectConflict("name", base.Name, ours.Name, theirs.Name); nameConflict != nil {
		...
		// If can't merge, keep ours (already set from copy above)
...
func (m *threeWayMerger) detectConflict(path string, base, ours, theirs any) *Conflict {
...
func (m *threeWayMerger) suggestResolution(path string, base, ours, theirs any) (bool, any) {
	// Don't use strategy for three-way merge conflict resolution
	// Strategies are for multi-source reconciliation, not three-way merging
	// Three-way merge should only auto-merge when changes don't conflict
...
func (m *threeWayMerger) mergePricing(base, ours, theirs *catalogs.ModelPricing) (*catalogs.ModelPricing, []Conflict) {
	...
	*merged = *ours // Start with our version
	...
	if ours.Tokens != nil || theirs.Tokens != nil {
		var baseTokens *catalogs.ModelTokenPricing
		if base.Tokens != nil {
...
func (m *threeWayMerger) mergeTokenPricing(base, ours, theirs *catalogs.ModelTokenPricing) (*catalogs.ModelTokenPricing, []Conflict) {
	conflicts := []Conflict{}
	...
	merged := &catalogs.ModelTokenPricing{}
	*merged = *ours // Start with our version

	if base == nil {
		base = &catalogs.ModelTokenPricing{}
	}

	// Input pricing
	if ours.Input != nil || theirs.Input != nil {
		var baseInput *catalogs.ModelTokenCost
		if base.Input != nil {
	...
	if ours.Output != nil || theirs.Output != nil {
		var baseOutput *catalogs.ModelTokenCost
		if base.Output != nil {
...
func (m *threeWayMerger) detectTokenCostConflict(path string, base, ours, theirs *catalogs.ModelTokenCost) *Conflict {
	// Convert to any for comparison
	var baseVal, oursVal, theirsVal any
	if base != nil {
		baseVal = base.Per1M
	}
	if ours != nil {
...
func tokenCostEqual(a, b *catalogs.ModelTokenCost) bool {
	if a == nil && b == nil {
		return true
	}
	if a == nil || b == nil {
	...
	for i, conflict := range conflicts {
		...
		case ResolutionAuthority:
			...
			if m.authorities != nil {
				authority := m.authorities.Find(conflict.Path, sources.ResourceTypeModel)
				if authority != nil {
