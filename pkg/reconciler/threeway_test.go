...
func TestThreeWayMergeNoConflicts(t *testing.T) {
	...
	merger := reconciler.NewThreeWayMerger(authorities, strategy)
...
func TestThreeWayMergeWithConflicts(t *testing.T) {
	...
	for _, conflict := range conflicts {
		if conflict.Type != reconciler.ConflictTypeModified {
	...
	// Debug output
	t.Logf("Base name: %q", base.Name)
	t.Logf("Ours name: %q", ours.Name)
	t.Logf("Theirs name: %q", theirs.Name)
	t.Logf("Merged name: %q", merged.Name)
...
func TestConflictResolution(t *testing.T) {
	...
	conflicts := []reconciler.Conflict{
		{
			...
			Ours:      "Ours",
			Theirs:    "Theirs",
			Type:      reconciler.ConflictTypeModified,
			CanMerge:  false,
			Suggested: nil,
		},
		{
	...
	strategies := []struct {
		strategy reconciler.ConflictResolution
		wantName string
		wantDesc string
	}{
		{
			strategy: reconciler.ResolutionOurs,
			wantName: "Ours",
			wantDesc: "Our Desc",
		},
		{
			strategy: reconciler.ResolutionTheirs,
			wantName: "Theirs",
			wantDesc: "Their Desc",
		},
		{
			strategy: reconciler.ResolutionBase,
			wantName: "Base",
			wantDesc: "Base Desc",
		},
		{
			strategy: reconciler.ResolutionMerge,
