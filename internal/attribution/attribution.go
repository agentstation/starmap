...
func buildAttributions(catalog *catalogs.Builder) (map[string][]catalogs.AuthorID, error) {
...
func processProviderOnlyAttributions(catalog *catalogs.Builder, attributions map[string][]catalogs.AuthorID) error {
...
func processProviderPatternAttributions(catalog *catalogs.Builder, attributions map[string][]catalogs.AuthorID) error {
	for _, author := range catalog.Authors().List() {
		// Check for nil pointers
		if author.Catalog == nil || author.Catalog.Attribution == nil {
			continue
...
func processGlobalPatternAttributions(catalog *catalogs.Builder, attributions map[string][]catalogs.AuthorID) error {
	for _, author := range catalog.Authors().List() {
		// Check for nil pointers
		if author.Catalog == nil || author.Catalog.Attribution == nil {
			continue
