...
type seamMatcher struct{}

func (seamMatcher) Match(input string) bool            { return input == "custom" }
func (seamMatcher) MatchAll(inputs ...string) []string { return inputs }
func (seamMatcher) MatchFirst(inputs ...string) string { return inputs[0] }
func (seamMatcher) MatchCount(inputs ...string) int    { return len(inputs) }
func (seamMatcher) Pattern() string                    { return "custom" }
func (seamMatcher) Type() PatternType                  { return Auto }

func TestSeamConformanceMultiMatcherAcceptsCustomAdapter(t *testing.T) {
	matcher, err := NewMultiMatcher(nil, Glob)
	if err != nil {
		t.Fatalf("NewMultiMatcher: %v", err)
	}
	matcher.AddMatcher(seamMatcher{})
	if !matcher.Match("custom") {
		t.Fatal("custom matcher adapter was not called")
	}
}
