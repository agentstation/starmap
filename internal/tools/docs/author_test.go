...
func TestGenerateAuthorIndex(t *testing.T) {
	...
	assert.Contains(t, contentStr, "🏢 Major Tech Companies")
	assert.Contains(t, contentStr, "🚀 AI Startups")
...
func TestGenerateAuthorModelPage(t *testing.T) {
	...
	// Check model details
	assert.Contains(t, contentStr, "# GPT-4")
	assert.Contains(t, contentStr, "## 📋 Technical Specifications")
	assert.Contains(t, contentStr, "| Specification | Value |")

	// Check capabilities
	assert.Contains(t, contentStr, "## 🎯 Capabilities")

	// Check providers section
	assert.Contains(t, contentStr, "## 🌐 Provider Availability")

	// Check related resources
	assert.Contains(t, contentStr, "### 🔗 Related Resources")
...
func TestCategorizeAuthor(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expected: "🎓 Research Organizations",
		},
		{
			name: "Unknown",
			author: &catalogs.Author{
