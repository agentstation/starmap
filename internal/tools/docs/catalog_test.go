...
func TestGenerateCatalogIndex(t *testing.T) {
	...
	assert.Contains(t, contentStr, "# 🌟 Starmap AI Model Catalog")
	...
	assert.Contains(t, contentStr, "## 📊 Catalog Statistics")
	...
	assert.Contains(t, contentStr, "## 🚀 Quick Navigation")
	...
	assert.Contains(t, contentStr, "## ⭐ Featured Models")
	assert.Contains(t, contentStr, "### Latest & Greatest")

	// Check browse options
	assert.Contains(t, contentStr, "## 📚 Browse Options")
...
func TestGenerateCatalogIndexWithManyAuthors(t *testing.T) {
	...
	assert.Contains(t, contentStr, "[View all 15 authors →](authors/)")
}
...
func TestGetProviderBadge(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name:     "Google",
			provider: "Google",
			expected: "🏢", // No specific entry for Google, uses default
		},
		{
			name:     "Unknown provider",
			provider: "Unknown",
			expected: "🏢", // Default badge
		},
	}
...
func TestGetAuthorBadge(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name:     "Unknown author",
			author:   "Unknown",
			expected: "👥", // Default badge for authors
		},
	}
