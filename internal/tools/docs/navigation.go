...
func (g *Generator) buildBreadcrumbs(items ...Breadcrumb) string {
	...
	for i, item := range items {
		if i == len(items)-1 {
			...
			parts[i] = fmt.Sprintf("**%s**", item.Label)
		} else {
			parts[i] = fmt.Sprintf("[%s](%s)", item.Label, item.Path)
		}
	}
...
// writeBreadcrumbs writes breadcrumb navigation to a file
func (g *Generator) writeBreadcrumbs(f *os.File, items ...Breadcrumb) {
...
func (g *Generator) buildFooter(backLinks ...Breadcrumb) string {
	...
	for _, link := range backLinks {
		parts = append(parts, fmt.Sprintf("[← %s](%s)", link.Label, link.Path))
	}
	
	// Always add the Starmap credit
	parts = append(parts, "Generated by [Starmap](https://github.com/agentstation/starmap)")
	
	return fmt.Sprintf("_%s_", strings.Join(parts, " | "))
}

// writeFooter writes a standard footer to a file
func (g *Generator) writeFooter(f *os.File, backLinks ...Breadcrumb) {
	fmt.Fprintln(f, "---")
	fmt.Fprintln(f, g.buildFooter(backLinks...))
}

// writeTimestampedFooter writes a footer with timestamp
func (g *Generator) writeTimestampedFooter(f *os.File, backLinks ...Breadcrumb) {
	fmt.Fprintln(f, "---")
	fmt.Fprintf(f, "_Last Updated: %s | %s_\n",
		time.Now().UTC().Format("2006-01-02 15:04:05 UTC"),
		g.buildFooter(backLinks...)[1:len(g.buildFooter(backLinks...))-1]) // Remove outer underscores
}
...
func (g *Generator) buildNavigationSection(title string, links []NavigationLink) string {
	...
	for _, link := range links {
		if link.Description != "" {
			sb.WriteString(fmt.Sprintf("- [%s](%s) - %s\n", link.Label, link.Path, link.Description))
		} else {
...
// writeNavigationSection writes a navigation section to a file
func (g *Generator) writeNavigationSection(f *os.File, title string, links []NavigationLink) {
	section := g.buildNavigationSection(title, links)
	if section != "" {
		fmt.Fprintln(f, section)
	}
}
