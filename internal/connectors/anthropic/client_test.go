		...
		t.Run(test.name, func(t *testing.T) {
			...
			provider := &catalogs.Provider{
				...
				Catalog: &catalogs.ProviderCatalog{Sources: []catalogs.ProviderSource{{
					...
					Auth: catalogs.ProviderAuthPolicy{Mode: catalogs.ProviderAuthModeNone}, Endpoint: catalogs.ProviderSourceEndpoint{Type: catalogs.EndpointTypeAnthropic, URL: server.URL},
				}}},
			}
...
func TestAnthropicDeterministicFixturePreservesListMetadata(t *testing.T) {
	response := loadTestdataResponse(t, "models_list.json")
	if len(response.Data) != 2 || response.Data[0].ID != "model-a" || response.Data[1].ID != "model-b" {
		t.Fatalf("deterministic response = %#v", response)
	}
}
