...
func newTestClient(t testing.TB, provider *catalogs.Provider) *Client {
	...
	if provider.Catalog.Sources[0].Endpoint.URL == "" && provider.Catalog.Sources[0].Endpoint.BaseURLEnv == "" {
		clone := catalogs.DeepCopyProvider(*provider)
		clone.Catalog.Sources[0].Endpoint.URL = "https://example.test/models"
		provider = &clone
	}
...
func loadTestdataResponse(t *testing.T, filename string) Response {
	...
	if err != nil {
		t.Fatalf("ReadFile connector testdata: %v", err)
	}
	var response Response
	if err := json.Unmarshal(payload, &response); err != nil {
		t.Fatalf("Unmarshal connector testdata: %v", err)
	}
	return response
}
...
func TestConvertToModelWithWildcardAuthorMapping(t *testing.T) {
		...
		Catalog: testProviderCatalog(catalogs.ProviderSourceEndpoint{
			...
			AuthorMapping: &catalogs.AuthorMapping{
				...
				Normalized: map[string]catalogs.AuthorID{
					"vendor-a/*":                    "author-a",
					"vendor-b/*":                    "author-b",
					"accounts/host/models/family*":  "author-family",
					"accounts/host/models/model*":   "author-model",
					"accounts/host/models/model-v*": "author-model-versioned",
				},
			},
		}),
	}
	...
	tests := []struct {
		...
		{
			name:   "first vendor namespace",
			id:     "vendor-a/model-one",
			author: "author-a",
		},
		{
			name:   "second vendor namespace",
			id:     "vendor-b/model-two",
			author: "author-b",
		},
		{
			name:   "hosted family path",
			id:     "accounts/host/models/family-large",
			author: "author-family",
		},
		{
			name:   "hosted model path",
			id:     "accounts/host/models/model-base",
			author: "author-model",
		},
		{
			name:   "longest matching path pattern wins",
			id:     "accounts/host/models/model-v2",
			author: "author-model-versioned",
		},
	}
...
func TestConvertToModelWithConfiguredNestedMetadata(t *testing.T) {
	...
	model := client.ConvertToModel(Model{
		ID:      "vendor-a/model-reasoning",
		Object:  "model",
		OwnedBy: "compatible",
		Metadata: &ModelMetadata{
			Description:   "Compatible reasoning model",
			ContextLength: &contextLength,
			Tags:          []string{"chat", "reasoning"},
		},
	})

	if model.Description != "Compatible reasoning model" {
	...
	if len(model.Authors) != 1 || model.Authors[0].ID != "author-a" {
		t.Fatalf("Expected configured author, got %#v", model.Authors)
	}
}
...
func TestConvertToModelPreservesConfiguredCompatibleDialectFields(t *testing.T) {
	provider := &catalogs.Provider{
		ID:   "compatible",
		Name: "Compatible Dialect",
		Catalog: testProviderCatalog(catalogs.ProviderSourceEndpoint{
	...
	model := client.ConvertToModel(Model{
		...
		HuggingFaceID:               "example/model-rich",
	...
	if model.Extensions["compatible"].Fields["hugging_face_id"] != "example/model-rich" {
		t.Fatalf("extensions = %#v", model.Extensions)
	}
}

func TestConvertToModelPreservesCompatibleMetadataAndPermissions(t *testing.T) {
	provider := &catalogs.Provider{
		ID:   "compatible",
		Name: "Compatible Dialect",
		Catalog: testProviderCatalog(catalogs.ProviderSourceEndpoint{
	...
	extension := model.Extensions["compatible"].Fields
...
func TestOpenAIClientListModels(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.URL.Path == "/v1/models" || r.URL.Path == "/":
			...
			if err != nil {
				t.Errorf("ReadFile connector testdata: %v", err)
				return
			}
			_, _ = w.Write(payload)
			return
	...
	provider := authenticatedTestProvider(server.URL)
	provider.Catalog.Sources[0].Endpoint.FeatureRules = []catalogs.FeatureRule{
		{
			Field:    "id",
			Contains: []string{"gpt-4"},
			Feature:  "tools",
...
func TestSchemaDriftMutationMatrix(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			client := newTestClient(t, &catalogs.Provider{
				ID: "test", Name: "Test",
				Catalog: testProviderCatalog(catalogs.ProviderSourceEndpoint{Type: catalogs.EndpointTypeOpenAI, URL: server.URL}),
			})
...
func TestConfiguredResponseModelsUsesValidatedCollectionPath(t *testing.T) {
	provider := &catalogs.Provider{Catalog: testProviderCatalog(catalogs.ProviderSourceEndpoint{ResponseCollection: "payload.models"})}
...
func TestConfiguredResponseModelsCoalescesOnlyByteIdenticalDuplicateRecords(t *testing.T) {
	provider := &catalogs.Provider{Catalog: testProviderCatalog(catalogs.ProviderSourceEndpoint{})}

	var identical Response
	if err := json.Unmarshal([]byte(`{"object":"list","data":[{"id":"model-a","object":"model"},{"id":"model-a","object":"model"}]}`), &identical); err != nil {
		t.Fatalf("decode identical fixture: %v", err)
	}
	models, err := configuredResponseModels(provider, identical)
	if err != nil {
		t.Fatalf("configuredResponseModels identical duplicates: %v", err)
	}
	if len(models) != 1 || models[0].ID != "model-a" {
		t.Fatalf("configuredResponseModels identical duplicates = %#v", models)
	}

	var conflicting Response
	if err := json.Unmarshal([]byte(`{"object":"list","data":[{"id":"model-a","object":"model","owned_by":"one"},{"id":"model-a","object":"model","owned_by":"two"}]}`), &conflicting); err != nil {
		t.Fatalf("decode conflicting fixture: %v", err)
	}
	if _, err := configuredResponseModels(provider, conflicting); err == nil {
		t.Fatal("configuredResponseModels accepted conflicting duplicate model records")
	}
}
