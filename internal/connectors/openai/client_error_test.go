	...
	tests := []struct {
	...
	}{
		{
			name: "source", mapping: catalogs.FieldMapping{
				From: "data[0].secret", To: "limits.context_window",
			}, wantField: "provider.catalog.endpoint.field_mappings[0].from",
		},
		{
...
func TestListModelsUsesImmutableResolvedConfigurationAfterCallerMutation(t *testing.T) {
	requestCount := 0
	...
	provider.Catalog.Sources[0].Endpoint.FieldMappings = []catalogs.FieldMapping{{
		From: "invalid[0]", To: "limits.context_window",
	}}

	_, err = client.ListModels(context.Background())
	require.NoError(t, err)
	assert.Equal(t, 1, requestCount, "client must use its immutable resolved configuration")
}
...
func TestClientModelConversion(t *testing.T) {
	...
	provider := authenticatedTestProvider("https://api.openai.com/v1/models")
...
func authenticatedTestProvider(endpoint string) *catalogs.Provider {
	provider := &catalogs.Provider{
		ID:   catalogs.ProviderIDOpenAI,
		Name: "OpenAI",
		Credentials: map[catalogs.ProviderCredentialID]catalogs.ProviderCredential{
			"api_key": {Env: catalogs.ProviderEnvironmentNames{"OPENAI_API_KEY"}},
		},
		Catalog: testProviderCatalog(catalogs.ProviderSourceEndpoint{Type: catalogs.EndpointTypeOpenAI, URL: endpoint}),
	}
	provider.Catalog.Sources[0].Auth = catalogs.ProviderAuthPolicy{Methods: []catalogs.ProviderCredentialID{"api_key"}}
	return provider
}

func testProviderCatalog(endpoint catalogs.ProviderSourceEndpoint) *catalogs.ProviderCatalog {
	return &catalogs.ProviderCatalog{Sources: []catalogs.ProviderSource{{
		ID:               "models",
