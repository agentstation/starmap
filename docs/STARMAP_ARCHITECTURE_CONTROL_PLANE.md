...
| [#52](https://github.com/agentstation/starmap/pull/52) | `codex/catalog-workspace-lifecycle@ba60db50` | `IN_PROGRESS` | P3 human-workspace lifecycle and explicit legacy-layout migration | Exact head passed local verification and hosted Security & Reliability; hosted Verification Gate exposed F-049 in the race harness/fixtures, whose repair must pass exact local and hosted gates before the protected merge pause |
