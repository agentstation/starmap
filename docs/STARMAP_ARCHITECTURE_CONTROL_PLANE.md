...
are complete. P4.8 now owns independent malformed-record quarantine after
observation health and non-authoritative absence were enforced.
...
| P4.8 | `IN_PROGRESS` | Quarantine records independently | Every P2.4-characterized whole-collection decode site (models.dev envelope, provider list responses, local YAML walk, stored payload) isolates a malformed record while valid siblings survive; collection envelope remains bounded |
