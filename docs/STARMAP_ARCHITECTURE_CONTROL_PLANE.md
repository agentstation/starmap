...
Status: `IN_PROGRESS` — P0 and P1 are complete. P2.2–P2.4 have pinned the
current workspace, store, reconciliation-loss, and whole-collection decode
defects; P2.5 is characterizing server and remote publication flow before
...
| P2.5 | `IN_PROGRESS` | Characterize server/remote flow | Real transport tests record manifest/payload behavior, absent Last-Event-ID handling, non-unique timestamp SSE IDs, opposite SSE/WebSocket backpressure, callback ordering, generation drop/reorder, and reconnect semantics |
