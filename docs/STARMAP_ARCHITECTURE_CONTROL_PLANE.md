...
Status: `IN_PROGRESS` — P0–P5 and every currently implementable P6 task are
complete. The public embeddable server and its third external module are now
real; P6.6 remains pending only on the public remote package owned by P7.9.
P7.3 is complete and P7.4 is the sole active task.
...
| P6.6 | `PENDING` | Validate complete concrete consumer matrix after P7.9 | Separate external test modules compile local library, store-only, server embed, and remote subscriber programs; P7.10 closes this task only after P7.1/P7.9 create the real public packages |
...
| F-064 | `PENDING` | Real `GOWORK=off` read-only, store-only, and server-embed modules have replaced three false journey goldens; the sketched remote import path still does not exist and requires the real P7.9 subscriber module before the four-composition gate can close | P6.6, P7.1, P7.9 |
