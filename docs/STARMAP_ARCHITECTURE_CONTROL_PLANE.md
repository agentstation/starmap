...
Status: `IN_PROGRESS` — P0–P5 and P6.1–P6.3 are complete. P6.4 is the sole
active task after the restored author/model catalog remained intact and the
small read-only library plus explicit acquisition boundary passed their exact
dependency, consumer, race, documentation, and performance gates.
...
| P6.4 | `IN_PROGRESS` | Narrow interfaces at use sites | Command, source, storage, server, and remote consumers define the smallest real role interfaces; the broad `internal/application.Application` interface is split by consumer |
