...
| [#52](https://github.com/agentstation/starmap/pull/52) | `codex/catalog-workspace-lifecycle@b3acac86` plus local F-050 remediation | `IN_PROGRESS` | P3 human-workspace lifecycle and explicit legacy-layout migration | Exact head `b3acac86` closed F-049 with both hosted checks; the outcome review then found and fixed F-050 before merge, and the remediation tree passes full local verification but must still be committed, repeat the exact-commit gate, and pass both hosted checks before the protected merge pause |
