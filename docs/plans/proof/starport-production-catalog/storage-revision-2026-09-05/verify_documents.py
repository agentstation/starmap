...
assert len(rows) == 31 and all(status == 'todo' for task, status, body in rows)
...
expected = {f'A{i:02}' for i in range(1, 44)}
assert acceptance['condition_count'] == 43
...
assert len(subcases) == len(set(subcases)) == 241
...
assert len(candidate) == 36 and len(published) == 7
...
assert len(prd_ids) == len(set(prd_ids)) == 33
assert len(spec_cases) == 43 and {case for case, requirements in spec_cases} == expected
assert set(prd_ids) == set(re.findall(r'P\d{2}', ' '.join(requirements for case, requirements in spec_cases)))
milestones = ['CSP7', 'CSP10', 'CSP15', 'CSP17', 'CSP21', 'CSP22', 'CSP23', 'CSP24']
...
assert list(phase_counts.values()) == [5, 21, 29, 33, 35, 36, 38, 43]
mapping = acceptance['storage_review_mapping']
assert set(mapping) == {f'SR{i:02}' for i in range(1, 15)}
...
documents = [prd, spec, root / 'docs/design/catalog-lifecycle/REPOSITORY_FINDINGS.md', root / 'docs/design/catalog-lifecycle/STORAGE_REVIEW.md', revision / 'REVIEW_RESOLUTION.md', proof / 'readme-demo-brief.md', root / 'docs/plans/README.md', root / 'docs/README.md']
...
json_files = list(proof.rglob('*.json'))
...
result = {
    ...
    'new_subcase_count': len(set(subcases) - oldsubcases), 'storage_findings_mapped': len(mapping),
    ...
    'no_product_source_changes': True, 'implementation_cases_run': 0, 'plan_status': 'proposed'
}
(proof / 'plan-validation.json').write_text(json.dumps(result, indent=2) + '\n')
